...
class ParticleClockFx extends ClockFx {
  ...
  void _activateParticle(Particle p) {
    ...
    double v = p.type == ParticleType.noise ? Rnd.getDouble(.5, 1) : Rnd.getDouble(.3, .4);
  ...
  double _getHourRadians() =>
      (time!.hour * pi / 6) + (time!.minute * pi / (6 * 60)) + (time!.second * pi / (360 * 60));

  /// Gets the radians of the minute hand.
  double _getMinuteRadians() => (time!.minute * (2 * pi) / 60) + (time!.second * pi / (30 * 60));
