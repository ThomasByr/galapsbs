...
typedef SizeIconBuilder<T> = Widget Function(
    BuildContext context, SizeProperties<T> local, DetailedGlobalToggleProperties<T> global);
...
typedef RollingIconBuilder<T> = Widget Function(
    BuildContext context, RollingProperties<T> local, DetailedGlobalToggleProperties<T> global);

typedef SimpleRollingIconBuilder<T> = Widget Function(T value, Size size, bool foreground);
...
typedef AnimatedIconBuilder<T> = Widget Function(
    BuildContext context, AnimatedToggleProperties<T> local, DetailedGlobalToggleProperties<T> global);
...
class AnimatedToggleSwitch<T> extends StatelessWidget {
  ...
  AnimatedToggleSwitch.size({
    ...
    this.draggingCursor = SystemMouseCursors.grabbing,
    this.dragCursor = SystemMouseCursors.grab,
  })  : animatedIconBuilder = _iconSizeBuilder<T>(iconBuilder, customIconBuilder, iconSize, selectedIconSize),
        this._iconArrangement = IconArrangement.row,
        super(key: key);
  ...
  AnimatedToggleSwitch.sizeByHeight({
    ...
    this.dragCursor = SystemMouseCursors.grab,
        ...
        animatedIconBuilder = _iconSizeBuilder<T>(iconBuilder, customIconBuilder,
            iconSize * (height + 2 * borderWidth), selectedIconSize * (height + 2 * borderWidth)),
        this._iconArrangement = IconArrangement.row,
        super(key: key);

  static AnimatedIconBuilder<T>? _iconSizeBuilder<T>(SimpleSizeIconBuilder<T>? iconBuilder,
      SizeIconBuilder<T>? customIconBuilder, Size iconSize, Size selectedIconSize) {
    ...
    return customIconBuilder == null
        ...
        : (context, local, global) => customIconBuilder!(
              ...
              SizeProperties.fromAnimated(
                  iconSize: Size(
                      iconSize.width + (selectedIconSize.width - iconSize.width) * local.animationValue,
                      iconSize.height + (selectedIconSize.height - iconSize.height) * local.animationValue),
                  properties: local),
              global,
            );
  }
  ...
  AnimatedToggleSwitch.rollingByHeight({
    ...
    this.dragCursor = SystemMouseCursors.grab,
        ...
        this.iconAnimationType = AnimationType.onSelected,
        this.foregroundIndicatorIconBuilder = _rollingForegroundIndicatorIconBuilder<T>(values, iconBuilder,
            customIconBuilder, Size.square(selectedIconRadius * 2 * (height - 2 * borderWidth))),
  ...
  AnimatedToggleSwitch.rolling({
    ...
    this.dragCursor = SystemMouseCursors.grab,
        ...
        this.selectedIconOpacity = iconOpacity,
        this.iconAnimationType = AnimationType.onSelected,
        this.foregroundIndicatorIconBuilder = _rollingForegroundIndicatorIconBuilder<T>(
            values, iconBuilder, customIconBuilder, Size.square(selectedIconRadius * 2)),
        this.animatedIconBuilder = _standardIconBuilder(
            iconBuilder, customIconBuilder, Size.square(iconRadius * 2), Size.square(iconRadius * 2)),
        this._iconArrangement = IconArrangement.row,
        super(key: key);

  static CustomIndicatorBuilder<T> _rollingForegroundIndicatorIconBuilder<T>(List<T> values,
      SimpleRollingIconBuilder<T>? iconBuilder, RollingIconBuilder<T>? customIconBuilder, Size iconSize) {
    assert(iconBuilder == null || customIconBuilder == null);
    if (customIconBuilder == null && iconBuilder != null)
      customIconBuilder = (c, l, g) => iconBuilder(l.value, l.iconSize, l.foreground);
    return (context, global) {
      if (customIconBuilder == null) return SizedBox();
      double distance = global.dif + global.indicatorSize.width;
      double angleDistance =
          distance / iconSize.longestSide * 2 * (global.textDirection == TextDirection.rtl ? -1.0 : 1.0);
  ...
  static AnimatedIconBuilder<T>? _standardIconBuilder<T>(SimpleRollingIconBuilder<T>? iconBuilder,
      RollingIconBuilder<T>? customIconBuilder, Size iconSize, Size selectedIconSize) {
    assert(iconBuilder == null || customIconBuilder == null);
    if (customIconBuilder == null && iconBuilder != null)
      customIconBuilder = (c, l, g) => iconBuilder(l.value, l.iconSize, l.foreground);
    return customIconBuilder == null
        ...
        : (t, local, global) => customIconBuilder!(
              t,
              RollingProperties.fromLocal(iconSize: iconSize, foreground: false, properties: local),
              global,
            );
  }
  ...
  AnimatedToggleSwitch.dual({
    ...
    bool opacityAnimation = true,
        ...
        this.foregroundIndicatorIconBuilder = _rollingForegroundIndicatorIconBuilder(
            [first, second],
  ...
  static Function() _dualOnTap<T>(Function(T)? onChanged, List<T> values, T current) {
    return () => onChanged?.call(values.firstWhere((element) => element != current));
  }
  ...
  static AnimatedIconBuilder<T>? _dualIconBuilder<T>(
    ...
    return (context, local, global) {
      ...
      AlignmentGeometry alignment = start ? AlignmentDirectional.centerStart : AlignmentDirectional.centerEnd;
  ...
  Widget build(BuildContext context) {
    ...
    BorderRadiusGeometry borderRadius = this.borderRadius ?? BorderRadius.circular(height / 2);
    ...
    return CustomAnimatedToggleSwitch<T>(
        ...
        backgroundIndicatorBuilder: foregroundIndicatorIconBuilder != null
            ? null
            : (context, properties) =>
                _indicatorBuilder(context, properties, indicatorColor, indicatorBorderRadius ?? borderRadius),
        foregroundIndicatorBuilder: foregroundIndicatorIconBuilder == null
            ? null
            : (context, properties) =>
                _indicatorBuilder(context, properties, indicatorColor, indicatorBorderRadius ?? borderRadius),
        iconBuilder: (context, local, global) =>
            _animatedOpacityIcon(_animatedSizeIcon(context, local, global), local.value == current),
        padding: EdgeInsets.all(borderWidth),
        wrapperBuilder: (context, properties, child) => TweenAnimationBuilder<Color?>(
              duration: animationDuration,
              tween: ColorTween(
                begin: borderColorBuilder?.call(current) ?? borderColor ?? theme.colorScheme.secondary,
                end: borderColorBuilder?.call(current) ?? borderColor ?? theme.colorScheme.secondary,
              ),
  ...
  Widget _indicatorBuilder(BuildContext context, DetailedGlobalToggleProperties<T> properties,
      ...
      case AnimationType.onSelected:
        ...
        return TweenAnimationBuilder<Color?>(
            ...
            builder: (c, color, child) =>
                _customIndicatorBuilder(properties.indicatorSize, color!, borderRadius, child));
      case AnimationType.onHover:
        return _customIndicatorBuilder(
            properties.indicatorSize,
            Color.lerp(colorBuilder?.call(values[pos.floor()]) ?? indicatorColor,
                    colorBuilder?.call(values[pos.ceil()]) ?? indicatorColor, pos - pos.floor()) ??
                indicatorColor,
            borderRadius,
    ...
    switch (iconAnimationType) {
      case AnimationType.onSelected:
        ...
        return TweenAnimationBuilder(
          ...
          tween: Tween<double>(begin: currentTweenValue, end: currentTweenValue),
          builder: (c, value, child) {
            return animatedIconBuilder!(
              c,
              AnimatedToggleProperties.fromLocal(animationValue: value as double, properties: local),
              global,
            );
          },
        );
      case AnimationType.onHover:
        ...
        double localPosition = global.position - global.position.floorToDouble();
        if (values[global.position.floor()] == local.value)
          animationValue = 1 - localPosition;
        else if (values[global.position.ceil()] == local.value) animationValue = localPosition;
        return animatedIconBuilder!(
          context,
          AnimatedToggleProperties.fromLocal(animationValue: animationValue, properties: local),
          global,
        );
    }
  }
  ...
  Widget _customIndicatorBuilder(Size size, Color color, BorderRadiusGeometry borderRadius, Widget? child) {
    return DecoratedBox(
      decoration: BoxDecoration(
        color: color,
        borderRadius: borderRadius,
